...
class AbliterateConfig:
    ...
    # 8 хватает, чтобы отделить «direction работает» от «нет» по short-gen
    # refusal-rate'у; дальнейшее увеличение тратит время без выигрыша.
    layer_search_prompts: int = 8
    # Сколько новых токенов генерим для «короткой» проверки-refusal при
    # поиске слоя. Refusal-эвристика срабатывает в первых 20-30 токенах,
    # 32 — безопасный потолок.
    layer_search_max_new_tokens: int = 32
    ...
    # для 28-слойной Qwen2.5-1.5B оптимум лежит в средних ~12 слоях, так
    # что режем 35% сверху и 20% снизу.
    skip_first_frac: float = 0.35
    skip_last_frac: float = 0.20
